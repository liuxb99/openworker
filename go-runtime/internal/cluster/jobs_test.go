...
func TestRemoteSubmitAndJobStatus(t *testing.T){
 var submitted model.SubmitRequest
 mux:=http.NewServeMux()
 mux.HandleFunc("/v1/node/status",func(w http.ResponseWriter,r *http.Request){json.NewEncoder(w).Encode(map[string]any{"node_id":"n1","machine":"HOST1","heartbeat_at":time.Now().UTC(),"lease_until":time.Now().UTC().Add(time.Minute),"max_workers":4,"busy_workers":0,"free_workers":4,"queued_jobs":0,"inventory":map[string]any{"capabilities":[]string{"x"}}})})
