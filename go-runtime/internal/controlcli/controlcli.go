...
func caseConfig(id string)(caseCfg,error){
    ...
    if strings.TrimSpace(h.CaseID)!=id{return caseCfg{},fmt.Errorf("case manifest identity mismatch requested=%q manifest=%q",id,h.CaseID)};if strings.TrimSpace(h.AssignedHost)==""{return caseCfg{},fmt.Errorf("case %s manifest missing assigned_host",id)};if strings.TrimSpace(h.WorkspaceRoot)==""||!filepath.IsAbs(h.WorkspaceRoot){return caseCfg{},fmt.Errorf("case %s manifest workspace_root must be absolute",id)};if h.Revision<=0{return caseCfg{},fmt.Errorf("case %s manifest revision must be positive",id)}
