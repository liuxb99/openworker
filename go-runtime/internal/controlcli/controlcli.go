...
type currentWorkRef struct { WorkID string `json:"work_id"`; StepID string `json:"step_id"`; ActionID string `json:"action_id"` }
...
func Run(program string, args []string, stdout, stderr io.Writer) int {
    ...
    case "supervisor status":
        if len(a)!=2{return usageCode(stderr,program)};m,e:=localMachine();if e!=nil{err=e;break};out,err=c.get("/api/execution/local-supervisor/status?machine="+url.QueryEscape(m)+"&limit=200")
    case "case status":
    ...
    case "case bootstrap":
        if len(a)!=3{return usageCode(stderr,program)};var cfg caseCfg;cfg,err=caseConfig(a[2]);if err!=nil{break};if err=requireLocalMachine(cfg.Machine);err!=nil{break};if err=c.requireOperational(cfg.Machine);err!=nil{break};out,err=c.post("/api/openworker/case/bootstrap",cfg.payload())
    case "case continue":
...
func(c client)caseStatus(cfg caseCfg)(any,error){out,err:=c.get("/api/openworker/case/status?case_id="+url.QueryEscape(cfg.CaseID)+"&machine="+url.QueryEscape(cfg.Machine)+"&workspace_root="+url.QueryEscape(cfg.Workspace));if err!=nil{return nil,err};m,ok:=out.(map[string]any);if !ok{return out,nil};controllerPath:=filepath.Join(cfg.Workspace,".openworker","case-controller-last.json");b,readErr:=os.ReadFile(controllerPath);if readErr!=nil{if !os.IsNotExist(readErr){m["current_work_read_error"]=readErr.Error()};return m,nil};var ref currentWorkRef;if err:=json.Unmarshal(b,&ref);err!=nil{m["current_work_read_error"]="decode case-controller-last.json: "+err.Error();return m,nil};if strings.TrimSpace(ref.WorkID)==""{return m,nil};m["current_work_ref"]=map[string]any{"work_id":ref.WorkID,"step_id":ref.StepID,"action_id":ref.ActionID};work,workErr:=c.get("/api/execution/local-work/"+url.PathEscape(ref.WorkID));if workErr!=nil{m["current_work_error"]=workErr.Error();return m,nil};m["current_work"]=work;return m,nil}
func caseConfig(id string)(caseCfg,error){id=strings.TrimSpace(id);root:=strings.TrimSpace(os.Getenv("OPENWORKER_ROOT"));if root==""{root=discoverRoot()};if root==""{return caseCfg{},fmt.Errorf("OpenWorker checkout not found")};switch id{case "0004":return caseCfg{"0004","DESKTOP-O87PJNR",`D:\AI-Work\jobs\0004-DWG-TO-3D`,root,filepath.Join(root,"case-worklists","0004.json"),filepath.Join(root,"case-specs","0004.json")},nil;case "0005":return caseCfg{"0005","DESKTOP-ODAQN0D",`D:\AI-Work\jobs\0005-SNOW-WHITE`,root,filepath.Join(root,"case-worklists","0005.json"),filepath.Join(root,"case-specs","0005.json")},nil;default:return caseCfg{},fmt.Errorf("unsupported case %q; fail-closed",id)}}
...
func discoverRoot()string{for _,p:=range[]string{`C:\github-runners\openworker\_work\openworker\openworker`,`D:\AI\openworker`,`D:\AIWork\openworker`,`D:\PyWork\openworker`}{if st,e:=os.Stat(filepath.Join(p,"case-worklists","0005.json"));e==nil&&!st.IsDir(){return p}};return ""}
func usageCode(w io.Writer,p string)int{usage(w,p);return 2}
func usage(w io.Writer,p string){fmt.Fprintf(w,"usage: %s supervisor status | case bootstrap 0004|0005 | case status 0004|0005 | case continue 0004|0005 | queue clear [MACHINE]\n",p)}
