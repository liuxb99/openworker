...
const (
	...
	AIOpenSeesReceiptSchema = "ai-opensees/operator-evidence/v0.4"
...
type AIOpenSeesOperatorEvidence struct {
	SchemaVersion string `json:"schema_version"`; CapabilityID string `json:"capability_id"`; Repository string `json:"repository"`; CommitSHA string `json:"commit_sha"`; RunID string `json:"run_id"`; RunAttempt string `json:"run_attempt"`; AssignedHostname string `json:"assigned_hostname"`; MCTPath string `json:"mct_path"`; MCTSHA256 string `json:"mct_sha256"`; RuntimeConfig string `json:"runtime_config"`; RuntimeConfigSHA256 string `json:"runtime_config_sha256"`; AuthorityGeneration int64 `json:"authority_generation"`; AuthorityCatalogRoot string `json:"authority_catalog_root"`; AuthorityEntryCount int `json:"authority_entry_count"`; AuthoritySnapshotSHA256 string `json:"authority_snapshot_sha256"`; OpenSeesExecutable string `json:"opensees_executable"`; OpenSeesExecutableSHA256 string `json:"opensees_executable_sha256"`; Workspace string `json:"workspace"`; Status string `json:"status"`; Artifacts []AIOpenSeesArtifact `json:"artifacts"`
}
type AIOpenSeesAnalysisResult struct {
	SchemaVersion string `json:"schema_version"`; Status string `json:"status"`; SolverExecutable string `json:"solver_executable"`; SourcePath string `json:"source_path"`; SourceSHA256 string `json:"source_sha256"`; AuthorityRuntimeUsed bool `json:"authority_runtime_used"`; AuthorityGeneration int64 `json:"authority_generation"`; AuthorityConfigPath string `json:"authority_config_path"`; AuthorityCatalogRoot string `json:"authority_catalog_root"`; AuthorityEntryCount int `json:"authority_entry_count"`; AuthoritySnapshotSHA256 string `json:"authority_snapshot_sha256"`; ScriptPath string `json:"script_path"`; ScriptSHA256 string `json:"script_sha256"`; StdoutPath string `json:"stdout_path"`; StdoutSHA256 string `json:"stdout_sha256"`; StderrPath string `json:"stderr_path"`; StderrSHA256 string `json:"stderr_sha256"`; GeometryJSONPath string `json:"geometry_json_path"`; GeometryJSONSHA256 string `json:"geometry_json_sha256"`; DeformedOBJPath string `json:"deformed_obj_path"`; DeformedOBJSHA256 string `json:"deformed_obj_sha256"`; DeformationSVGPath string `json:"deformation_svg_path"`; DeformationSVGSHA256 string `json:"deformation_svg_sha256"`; DisplacementCSVPath string `json:"displacement_csv_path"`; DisplacementCSVSHA256 string `json:"displacement_csv_sha256"`; ReactionCSVPath string `json:"reaction_csv_path"`; ReactionCSVSHA256 string `json:"reaction_csv_sha256"`
}
type AIOpenSeesRuntimeState struct { SchemaVersion string `json:"schema_version"`; Ready bool `json:"ready"`; Generation int64 `json:"generation"`; ConfigPath string `json:"config_path"`; CatalogRoot string `json:"catalog_root"`; EntryCount int `json:"entry_count"`; SnapshotSHA256 string `json:"snapshot_sha256"`; SnapshotValid bool `json:"snapshot_valid"` }
...
func ValidateAIOpenSeesWorkspace(workspace string) AIOpenSeesEvidenceReport {
	report:=AIOpenSeesEvidenceReport{SchemaVersion:"openworker/ai-opensees-evidence-report/v0.4",Workspace:filepath.Clean(workspace),Blockers:[]string{}}
	...
	if receipt.SchemaVersion!=AIOpenSeesReceiptSchema{add("OPERATOR_EVIDENCE_SCHEMA_MISMATCH")};if receipt.CapabilityID!=AIOpenSeesCapabilityID{add("CAPABILITY_ID_MISMATCH")};if receipt.Repository!=AIOpenSeesRepository{add("REPOSITORY_MISMATCH")};if !isGitCommitSHA(receipt.CommitSHA){add("COMMIT_SHA_INVALID")};if !isPositiveDecimal(receipt.RunID){add("RUN_ID_INVALID")};if !isPositiveDecimal(receipt.RunAttempt){add("RUN_ATTEMPT_INVALID")};if !strings.EqualFold(receipt.AssignedHostname,AIOpenSeesHost){add("ASSIGNED_HOST_MISMATCH")};if receipt.Status!="complete"{add("OPERATOR_STATUS_NOT_COMPLETE")};if receipt.AuthorityGeneration<1{add("AUTHORITY_GENERATION_INVALID")};if receipt.AuthorityEntryCount<0{add("AUTHORITY_ENTRY_COUNT_INVALID")};if strings.TrimSpace(receipt.AuthorityCatalogRoot)==""{add("AUTHORITY_CATALOG_ROOT_EMPTY")};if !isSHA256(receipt.AuthoritySnapshotSHA256){add("AUTHORITY_SNAPSHOT_SHA256_INVALID")};if !isSHA256(receipt.MCTSHA256){add("MCT_SHA256_INVALID")};if !isSHA256(receipt.RuntimeConfigSHA256){add("RUNTIME_CONFIG_SHA256_INVALID")};if !isSHA256(receipt.OpenSeesExecutableSHA256){add("OPENSEES_EXECUTABLE_SHA256_INVALID")};if strings.TrimSpace(receipt.MCTPath)==""{add("MCT_PATH_EMPTY")};if strings.TrimSpace(receipt.RuntimeConfig)==""{add("RUNTIME_CONFIG_EMPTY")};if strings.TrimSpace(receipt.OpenSeesExecutable)==""{add("OPENSEES_EXECUTABLE_EMPTY")};if !samePath(receipt.Workspace,workspace){add("WORKSPACE_RECEIPT_MISMATCH")}
	if strings.TrimSpace(receipt.MCTPath)!=""{actual,bytes,err:=sha256File(receipt.MCTPath);if err!=nil{add("MCT_READ_FAILED:"+err.Error())}else{if bytes<=0{add("MCT_EMPTY")};if !strings.EqualFold(actual,receipt.MCTSHA256){add("MCT_FILE_SHA256_MISMATCH")}}}
	...
	if result.SchemaVersion!=AIOpenSeesResultSchema{add("ANALYSIS_RESULT_SCHEMA_MISMATCH")};if result.Status!="complete"{add("ANALYSIS_STATUS_NOT_COMPLETE")};if !result.AuthorityRuntimeUsed{add("ANALYSIS_AUTHORITY_RUNTIME_NOT_USED")};if !samePath(result.SourcePath,receipt.MCTPath){add("ANALYSIS_SOURCE_PATH_MISMATCH")};if !strings.EqualFold(result.SourceSHA256,receipt.MCTSHA256){add("SOURCE_SHA256_MISMATCH")};if !samePath(result.AuthorityConfigPath,receipt.RuntimeConfig){add("ANALYSIS_RUNTIME_CONFIG_PATH_MISMATCH")};if !samePath(result.SolverExecutable,receipt.OpenSeesExecutable){add("ANALYSIS_OPENSEES_EXECUTABLE_PATH_MISMATCH")};if result.AuthorityGeneration!=receipt.AuthorityGeneration{add("AUTHORITY_GENERATION_MISMATCH")};if !samePath(result.AuthorityCatalogRoot,receipt.AuthorityCatalogRoot){add("ANALYSIS_CATALOG_ROOT_MISMATCH")};if result.AuthorityEntryCount!=receipt.AuthorityEntryCount{add("ANALYSIS_ENTRY_COUNT_MISMATCH")};if !isSHA256(result.AuthoritySnapshotSHA256)||!strings.EqualFold(result.AuthoritySnapshotSHA256,receipt.AuthoritySnapshotSHA256){add("ANALYSIS_SNAPSHOT_SHA256_MISMATCH")}
	if runtime.SchemaVersion!=AIOpenSeesRuntimeSchema{add("AUTHORITY_RUNTIME_SCHEMA_MISMATCH")};if !runtime.Ready||!runtime.SnapshotValid{add("AUTHORITY_RUNTIME_NOT_READY")};if !samePath(runtime.ConfigPath,receipt.RuntimeConfig){add("RUNTIME_CONFIG_PATH_MISMATCH")};if runtime.Generation!=receipt.AuthorityGeneration{add("RUNTIME_GENERATION_MISMATCH")};if !samePath(runtime.CatalogRoot,receipt.AuthorityCatalogRoot){add("RUNTIME_CATALOG_ROOT_MISMATCH")};if runtime.EntryCount!=receipt.AuthorityEntryCount{add("RUNTIME_ENTRY_COUNT_MISMATCH")};if !isSHA256(runtime.SnapshotSHA256)||!strings.EqualFold(runtime.SnapshotSHA256,receipt.AuthoritySnapshotSHA256){add("RUNTIME_SNAPSHOT_SHA256_MISMATCH")}
