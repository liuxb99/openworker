...
func TestValidateAIOpenSeesWorkspace(t *testing.T) {
	...
	missingActiveSection:=receipt; missingActiveSection.ActiveSourcePrismaticSectionAuthorityCount=0; missingActiveSection.ActiveSourceAuthorityCount=2; writeReceipt(missingActiveSection); if r:=ValidateAIOpenSeesWorkspace(root); r.Accepted||!hasBlocker(r,"ACTIVE_SOURCE_SECTION_AUTHORITY_COVERAGE_MISSING") { t.Fatalf("missing active-source SECTION authority not blocked: %v",r.Blockers) }
	writeReceipt(receipt)
	runtimeSourceDrift:=runtime; runtimeSourceDrift.ActiveSourceSHA256=testHash([]byte("other-source")); writeRuntime(runtimeSourceDrift); if r:=ValidateAIOpenSeesWorkspace(root); r.Accepted||!hasBlocker(r,"RUNTIME_ACTIVE_SOURCE_SHA256_MISMATCH") { t.Fatalf("runtime active source drift not blocked: %v",r.Blockers) }; writeRuntime(runtime)
