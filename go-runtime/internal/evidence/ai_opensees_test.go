...
func TestValidateAIOpenSeesWorkspace(t *testing.T) {
	...
	runtimeConfigData:=[]byte("schema_version=ai-opensees/mct-authority-runtime-config/v0.1\ncatalog_root=authority-catalog\nrequire_valid_snapshot=true\n"); runtimeConfigPath:=filepath.Join(root,"runtime.config"); testWrite(t,runtimeConfigPath,runtimeConfigData)
	openSeesData:=[]byte("real-opensees-executable-placeholder"); openSeesPath:=filepath.Join(root,"OpenSees.exe"); testWrite(t,openSeesPath,openSeesData)
	snapshotSHA:=testHash([]byte("authority-snapshot")); generation:=int64(3); catalogRoot:=filepath.Join(root,"authority-catalog")
	if err:=os.MkdirAll(catalogRoot,0o755);err!=nil{t.Fatal(err)}
	runtime:=AIOpenSeesRuntimeState{SchemaVersion:AIOpenSeesRuntimeSchema,Ready:true,Generation:generation,ConfigPath:runtimeConfigPath,CatalogRoot:catalogRoot,EntryCount:3,SnapshotSHA256:snapshotSHA,SnapshotValid:true}
	...
	receipt:=AIOpenSeesOperatorEvidence{SchemaVersion:AIOpenSeesReceiptSchema,CapabilityID:AIOpenSeesCapabilityID,Repository:AIOpenSeesRepository,CommitSHA:testHash([]byte("commit")),RunID:"123",RunAttempt:"1",AssignedHostname:AIOpenSeesHost,MCTPath:mctPath,MCTSHA256:mctSHA,RuntimeConfig:runtimeConfigPath,RuntimeConfigSHA256:testHash(runtimeConfigData),AuthorityGeneration:generation,AuthorityCatalogRoot:catalogRoot,AuthorityEntryCount:3,AuthoritySnapshotSHA256:snapshotSHA,OpenSeesExecutable:openSeesPath,OpenSeesExecutableSHA256:testHash(openSeesData),Workspace:root,Status:"complete",Artifacts:artifacts}
	...
	testWrite(t,mctPath,mctData)
	testWrite(t,runtimeConfigPath,[]byte("tampered-runtime-config"))
	configTampered:=ValidateAIOpenSeesWorkspace(root);if configTampered.Accepted{t.Fatal("tampered authority runtime config must not be accepted")};found=false;for _,blocker:=range configTampered.Blockers{if blocker=="RUNTIME_CONFIG_FILE_SHA256_MISMATCH"{found=true;break}};if !found{t.Fatalf("expected runtime config hash blocker, got %v",configTampered.Blockers)}

	testWrite(t,runtimeConfigPath,runtimeConfigData)
	testWrite(t,openSeesPath,[]byte("tampered-opensees-executable"))
	solverTampered:=ValidateAIOpenSeesWorkspace(root);if solverTampered.Accepted{t.Fatal("tampered OpenSees executable must not be accepted")};found=false;for _,blocker:=range solverTampered.Blockers{if blocker=="OPENSEES_EXECUTABLE_FILE_SHA256_MISMATCH"{found=true;break}};if !found{t.Fatalf("expected OpenSees executable hash blocker, got %v",solverTampered.Blockers)}

	testWrite(t,openSeesPath,openSeesData)
	pathDriftAnalysis:=analysis;pathDriftAnalysis.AuthorityConfigPath=filepath.Join(root,"other-runtime.config");pathDriftData,err:=json.Marshal(pathDriftAnalysis);if err!=nil{t.Fatal(err)};testWrite(t,filepath.Join(root,"analysis-result.json"),pathDriftData)
	pathDrift:=ValidateAIOpenSeesWorkspace(root);if pathDrift.Accepted{t.Fatal("authority runtime config path drift must not be accepted")};found=false;for _,blocker:=range pathDrift.Blockers{if blocker=="ANALYSIS_RUNTIME_CONFIG_PATH_MISMATCH"{found=true;break}};if !found{t.Fatalf("expected authority config path blocker, got %v",pathDrift.Blockers)}
}
