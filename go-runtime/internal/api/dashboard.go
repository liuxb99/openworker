...
<style>
body{font-family:system-ui,-apple-system,"Segoe UI",sans-serif;margin:0;background:#111827;color:#e5e7eb}header{padding:18px 22px;background:#0b1220;position:sticky;top:0}h1{font-size:20px;margin:0 0 10px}.bar{display:flex;gap:8px;flex-wrap:wrap}input,select,button{background:#1f2937;color:#e5e7eb;border:1px solid #374151;border-radius:7px;padding:8px}button{cursor:pointer}main{padding:18px}.meta{color:#9ca3af;margin-bottom:12px}.grid{display:grid;grid-template-columns:minmax(0,1.5fr) minmax(320px,1fr);gap:14px}.panel{background:#172033;border:1px solid #27344a;border-radius:10px;overflow:hidden}table{width:100%;border-collapse:collapse;font-size:13px}th,td{padding:9px;border-bottom:1px solid #27344a;text-align:left;vertical-align:top}tr:hover{background:#1d2940}.status{font-weight:700}.running,.starting{color:#fbbf24}.succeeded{color:#34d399}.failed,.timed_out{color:#fb7185}.queued_local,.accepted{color:#60a5fa}pre{white-space:pre-wrap;word-break:break-word;margin:0;padding:12px;font-size:12px;max-height:72vh;overflow:auto}.small{font-size:12px;color:#9ca3af}@media(max-width:900px){.grid{grid-template-columns:1fr}}
</style></head>
...
<main><div class="meta" id="meta">載入中…</div><div class="grid"><div class="panel"><table><thead><tr><th>狀態</th><th>Job / Work</th><th>Slot/PID</th><th>時間</th></tr></thead><tbody id="jobs"></tbody></table></div><div class="panel"><pre id="detail">點選左側工作查看完整記錄與 events。</pre></div></div></main>
<script>
