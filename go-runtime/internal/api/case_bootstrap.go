...
func (s *Server) caseBootstrap(w http.ResponseWriter,r *http.Request){
    ...
    if req.CaseID!="0005"{s.bootstrapFail(w,400,req,"validate_case","validate native Go case allowlist",fmt.Errorf("unsupported native Go case %q",req.CaseID),"use an implemented native Go case controller",checks);return}
    ...
    if _,err:=requireBootstrapFile(root,req.SpecPath);err!=nil{s.bootstrapFail(w,400,req,"validate_spec","verify case spec",err,"sync/fix case spec and retry",checks);return}
    result,err:=casecontroller.Bootstrap(req.CaseID,s.machine,workspace,manifest);if err!=nil{s.bootstrapFail(w,409,req,"go_native_bootstrap","run native Go case bootstrap",err,"repair native Go controller inputs and retry",checks);return}
