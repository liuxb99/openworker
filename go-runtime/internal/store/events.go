...
func (s *Store) RecordEvent(jobID, eventType, detail string) error {
	_, err := s.db.Exec(`INSERT INTO job_events(job_id,event_type,detail,created_at) VALUES(?,?,?,?)`, jobID, eventType, detail, nowText(time.Now().UTC()))
	return err
}
