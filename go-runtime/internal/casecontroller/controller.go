...
type Step struct { StepID string `json:"step_id"`; Dependencies []string `json:"dependencies"`; Status string `json:"status"`; Evidence map[string]any `json:"evidence"` }
...
type BootstrapResult struct { Schema string `json:"schema"`; CaseID string `json:"case_id"`; Machine string `json:"machine"`; WorkspaceRoot string `json:"workspace_root"`; Revision int `json:"revision"`; ReadyStepIDs []string `json:"ready_step_ids"`; LedgerPath string `json:"ledger_path"`; WorklistSnapshot string `json:"worklist_snapshot"`; ControllerSnapshot string `json:"controller_snapshot"`; Controller string `json:"controller"`; PythonRequired bool `json:"python_required"`; CompletedAt time.Time `json:"completed_at"` }
type ledgerEvent struct { Schema string `json:"schema"`; Timestamp time.Time `json:"timestamp"`; CaseID string `json:"case_id"`; Machine string `json:"machine"`; EventType string `json:"event_type"`; WorkspaceRoot string `json:"workspace_root"`; Revision int `json:"revision,omitempty"`; ReadyStepIDs []string `json:"ready_step_ids,omitempty"`; Detail string `json:"detail,omitempty"` }
...
func Bootstrap(caseID,machine,workspaceRoot,manifestPath string)(BootstrapResult,error){
    ...
    ledger:=filepath.Join(marker,"case-supervisor-ledger.jsonl");worklistSnapshot:=filepath.Join(marker,"case-worklist.json");controllerSnapshot:=filepath.Join(marker,"case-controller-last.json");ready:=readySteps(w.Steps);now:=time.Now().UTC()
    ...
    if err:=atomicWrite(worklistSnapshot,b);err!=nil{return BootstrapResult{},fmt.Errorf("write worklist snapshot: %w",err)}
    result:=BootstrapResult{Schema:Schema,CaseID:caseID,Machine:machine,WorkspaceRoot:workspaceRoot,Revision:w.Revision,ReadyStepIDs:ready,LedgerPath:ledger,WorklistSnapshot:worklistSnapshot,ControllerSnapshot:controllerSnapshot,Controller:"go-native",PythonRequired:false,CompletedAt:time.Now().UTC()}
...
func readySteps(steps []Step)[]string{done:=map[string]bool{};for _,s:=range steps{if strings.EqualFold(s.Status,"SUCCEEDED")||strings.EqualFold(s.Status,"COMPLETED"){done[s.StepID]=true}};out:=[]string{};for _,s:=range steps{if !strings.EqualFold(s.Status,"PENDING"){continue};ok:=true;for _,d:=range s.Dependencies{if !done[d]{ok=false;break}};if ok{out=append(out,s.StepID)}};return out}
