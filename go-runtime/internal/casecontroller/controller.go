...
func readySteps(steps []Step)[]string{done:=map[string]bool{};for _,s:=range steps{if strings.EqualFold(s.Status,"SUCCEEDED")||strings.EqualFold(s.Status,"COMPLETED")||strings.EqualFold(s.Status,"PASSED"){done[s.StepID]=true}};out:=[]string{};for _,s:=range steps{if !strings.EqualFold(s.Status,"PENDING"){continue};ok:=true;for _,d:=range s.Dependencies{if !done[d]{ok=false;break}};if ok{out=append(out,s.StepID)}};return out}
