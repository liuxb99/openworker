...
var actionMappers = map[string]actionMapper{
	...
	"comfyx-studio.storyboard.real-bind":    mapStoryboardRealBind,
...
func mapActionInputs(step *Step, w Worklist, workspaceRoot, machine, specPath string) (string, map[string]any, error) {
	if step == nil { return "", nil, fmt.Errorf("step is required") }
	if len(step.AllowedActions) != 1 { return "", nil, fmt.Errorf("step %s requires exactly one allowed action", step.StepID) }
	action := strings.TrimSpace(step.AllowedActions[0]); if action == "" { return "", nil, fmt.Errorf("step %s has empty allowed action", step.StepID) }
	mapper, ok := actionMappers[action]; if !ok { return "", nil, fmt.Errorf("no registered action mapper for capability %q", action) }
	inputs, err := mapper(actionMapContext{Step: step, Worklist: w, WorkspaceRoot: workspaceRoot, Machine: machine, SpecPath: specPath})
	if err != nil { return "", nil, fmt.Errorf("map capability %s for step %s: %w", action, step.StepID, err) }
	dispatchAction := action; if alias := strings.TrimSpace(actionDispatchAliases[action]); alias != "" { dispatchAction = alias }
	return dispatchAction, inputs, nil
}

func terminalSuccess(status string) bool { return strings.EqualFold(status,"SUCCEEDED")||strings.EqualFold(status,"PASSED")||strings.EqualFold(status,"COMPLETED") }

func dependencyStep(ctx actionMapContext, index int) (*Step, error) {
	if index < 0 || index >= len(ctx.Step.Dependencies) { return nil, fmt.Errorf("step %s dependency[%d] unavailable", ctx.Step.StepID, index) }
	id := ctx.Step.Dependencies[index]; parent := findStep(ctx.Worklist.Steps, id); if parent == nil { return nil, fmt.Errorf("dependency %s missing from worklist", id) }
	if !terminalSuccess(parent.Status) { return nil, fmt.Errorf("dependency %s is not terminal-success: %s", id, parent.Status) }
	return parent, nil
}

func evidenceFile(ctx actionMapContext, parent *Step, key string) (string, error) {
	raw := strings.TrimSpace(fmt.Sprint(parent.Evidence[key])); if raw == "" { return "", fmt.Errorf("dependency %s evidence missing %s", parent.StepID, key) }
	return workspaceRelativeExistingFile(ctx.WorkspaceRoot, raw, key)
}

func uniqueSucceededEvidenceFile(ctx actionMapContext,key string)(string,error){
	matches:=[]string{}
	for i:=range ctx.Worklist.Steps{
		s:=&ctx.Worklist.Steps[i];if !terminalSuccess(s.Status){continue}
		raw:=strings.TrimSpace(fmt.Sprint(s.Evidence[key]));if raw==""{continue}
		rel,err:=workspaceRelativeExistingFile(ctx.WorkspaceRoot,raw,key);if err!=nil{return "",fmt.Errorf("step %s %s: %w",s.StepID,key,err)}
		matches=append(matches,rel)
	}
	if len(matches)!=1{return "",fmt.Errorf("expected exactly one succeeded %s evidence file, got %d",key,len(matches))}
	return matches[0],nil
}

func mapStoryIndex(ctx actionMapContext) (map[string]any, error) {
	spec, err := readCaseSpec(ctx.SpecPath, ctx.Worklist.CaseID); if err != nil { return nil, err }
...
func mapDirectorPreproduction(ctx actionMapContext) (map[string]any, error) {
	spec, err := readCaseSpec(ctx.SpecPath, ctx.Worklist.CaseID); if err != nil { return nil, err }
	title := strings.TrimSpace(fmt.Sprint(spec["title"])); story := strings.TrimSpace(fmt.Sprint(spec["source_story"])); if title == "" || story == "" { return nil, fmt.Errorf("director inputs require title and source_story") }
...
func mapStoryboardRealBind(ctx actionMapContext)(map[string]any,error){
	for i:=range ctx.Step.Dependencies{if _,err:=dependencyStep(ctx,i);err!=nil{return nil,err}}
	requestRel,err:=uniqueSucceededEvidenceFile(ctx,"storyboard_request");if err!=nil{return nil,err}
	return map[string]any{
		"workspace_root":ctx.WorkspaceRoot,
		"assigned_host":ctx.Machine,
		"request_relpath":requestRel,
		"output_relpath":filepath.ToSlash(filepath.Join("presentation","storyboard-request.bound.json")),
	},nil
}
...
func mapCasePublishArtifacts(ctx actionMapContext) (map[string]any, error) {
	parent, err := dependencyStep(ctx, 0); if err != nil { return nil, err }
	keys := []string{"storyboard_pptx","storyboard_manifest","reopen_receipt"}; artifacts := make([]string, 0, len(keys))
	for _, key := range keys { rel, err := evidenceFile(ctx, parent, key); if err != nil { return nil, err }; artifacts = append(artifacts, filepath.ToSlash(rel)) }
...
func mapDriveApprovalGate(ctx actionMapContext) (map[string]any, error) {
	parent, err := dependencyStep(ctx, 0); if err != nil { return nil, err }
	if strings.TrimSpace(fmt.Sprint(parent.Evidence["drive_folder_id"])) == "" || strings.TrimSpace(fmt.Sprint(parent.Evidence["manifest_sha256"])) == "" { return nil, fmt.Errorf("publish dependency evidence incomplete") }
