...
func main(){
 listen:=flag.String("listen","127.0.0.1:8850","MCP listen address; localhost only")
 flag.Parse()
...
func(b *bridge)health(w http.ResponseWriter,r *http.Request){if r.Method!=http.MethodGet{http.Error(w,"GET only",405);return};writeJSON(w,200,map[string]any{"status":"ok","authority":"openworker-opencode-mcp","transport":"localhost-opencode","github_action_used_for_business_execution":false})}
func(b *bridge)mcp(w http.ResponseWriter,r *http.Request){if r.Method!=http.MethodPost{http.Error(w,"POST only",405);return};if !b.authorized(r){http.Error(w,"unauthorized",401);return};var q rpcRequest;if err:=json.NewDecoder(http.MaxBytesReader(w,r.Body,1<<20)).Decode(&q);err!=nil{b.rpcErr(w,nil,-32700,"parse error",err.Error());return};if q.JSONRPC!="2.0"{b.rpcErr(w,q.ID,-32600,"invalid request","jsonrpc must be 2.0");return}
 switch q.Method{
 case "initialize": writeJSON(w,200,rpcResponse{JSONRPC:"2.0",ID:q.ID,Result:map[string]any{"protocolVersion":"2025-11-25","capabilities":map[string]any{"tools":map[string]any{"listChanged":false}},"serverInfo":map[string]any{"name":"openworker-opencode-bridge","version":"1.0.0"}}})
...
func toolList()[]any{return []any{
 ...
 map[string]any{"name":"case_continue","description":"Continue Case 0005 on its fixed ODA local supervisor. This is a write action and never uses GitHub Actions.","inputSchema":map[string]any{"type":"object","properties":map[string]any{"case_id":map[string]any{"type":"string","enum":[]string{"0005"}}},"required":[]string{"case_id"},"additionalProperties":false},"annotations":map[string]any{"readOnlyHint":false,"destructiveHint":false}},
...
func(b *bridge)callTool(tc toolCall)(string,error){var args []string;switch tc.Name{
 case "supervisor_status":if len(tc.Arguments)!=0{return "",errors.New("supervisor_status takes no arguments")};args=[]string{"supervisor","status"}
 case "case_status":if fmt.Sprint(tc.Arguments["case_id"])!="0005"{return "",errors.New("only case 0005 is allowed")};args=[]string{"case","status","0005"}
 case "case_continue":if fmt.Sprint(tc.Arguments["case_id"])!="0005"{return "",errors.New("only case 0005 is allowed")};args=[]string{"case","continue","0005"}
 case "queue_clear":if !strings.EqualFold(fmt.Sprint(tc.Arguments["machine"]),"DESKTOP-ODAQN0D"){return "",errors.New("only DESKTOP-ODAQN0D is allowed")};args=[]string{"queue","clear","DESKTOP-ODAQN0D"}
 default:return "",fmt.Errorf("tool %q is not allowlisted",tc.Name)}
 cmd:=quotePS(b.ctl)+" "+strings.Join(args," ");return b.runViaOpenCode(cmd)}
