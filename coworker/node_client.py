"""Thin Python control-plane adapter for the local OpenWorker Go execution node."""
...
class OpenWorkerNodeClient:
    ...
    def drain(self, mode: str = "queued") -> dict[str, Any]:
        if mode not in {"queued", "all"}:
            raise ValueError("mode must be queued or all")
