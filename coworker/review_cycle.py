...
class ReviewCycle:
    ...
    def build_bundle(
        ...
        *,
        ...
        staging = Path(tempfile.mkdtemp(prefix=f"{revision_id}-", dir=str(self.review_dir.parent if self.review_dir.parent.exists() else self.openworker_dir)))
        ...
        try:
            for item in artifacts:
                ...
                safe_name = _safe_name(logical) + suffix
    ...
    def handoff_to_drive_sync(
        ...
        *,
        ...
        """Atomically copy a bundle into a Google Drive for desktop synced folder."""
        source = Path(bundle_root).expanduser().resolve()
        if not source.is_dir():
            raise ReviewCycleError(f"review bundle unavailable: {source}")
        configured = str(drive_sync_root or os.environ.get("OPENWORKER_REVIEW_DRIVE_ROOT") or "").strip()
        if not configured:
            raise ReviewCycleError(
                "Google Drive handoff is not configured; set OPENWORKER_REVIEW_DRIVE_ROOT to the local synced folder"
            )
...
__all__ = ["DEFAULT_DRIVE_FOLDER_ID", "ReviewArtifact", "ReviewCycle", "ReviewCycleError"]
