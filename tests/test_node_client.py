...
def test_node_client_contract(monkeypatch):
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path == "/v1/node/status":
            return httpx.Response(200, json={"machine": "TESTHOST", "online": True})
        if request.url.path == "/v1/jobs" and request.method == "POST":
            return httpx.Response(202, json={"job_id": "OWJ-1", "accepted": True})
        if request.url.path.endswith("/retry"):
            return httpx.Response(202, json={"job_id": "OWJ-1", "status": "queued_local"})
        if request.url.path == "/v1/queue/drain":
            return httpx.Response(200, json={"ok": True, "mode": request.url.params["mode"]})
