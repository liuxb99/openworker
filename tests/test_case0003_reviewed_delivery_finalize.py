...
def _fixture(tmp_path: Path):
    ...
    bundle_manifest.write_text(json.dumps({"schema_version": "openworker-review-bundle/v1", "revision_id": revision_id}), encoding="utf-8")
    ...
    return workspace, revision_id, bundle_manifest


def test_finalize_binds_accepted_review_to_current_os_and_drive(tmp_path: Path):
    workspace, revision_id, _ = _fixture(tmp_path)
    ...
    try:
        ...
        assert delivered[0]["payload"]["engineering_os"]["delivery_id"] == "del_current"
        assert delivered[0]["payload"]["engineering_os"]["delivery_revision"] == 7
        assert delivered[0]["payload"]["google_drive"]["drive_file_id"] == "drive-file"
    finally:
        ledger.close()


def test_finalize_rejects_stale_review_bundle_without_moving_delivery_pointer(tmp_path: Path):
    workspace, revision_id, bundle_manifest = _fixture(tmp_path)
