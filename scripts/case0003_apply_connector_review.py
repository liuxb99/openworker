"""Apply a ChatGPT review that was grounded by the connected Google Drive.

The connector performs the cloud read/write outside the UL7 runner. This script
only accepts a durable receipt after the exact cloud ZIP has been read back and
its immutable manifest SHA has been verified. WorkLedger remains authoritative.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    receipt = _load(receipt_path, "connector review receipt")
    if str(receipt.get("revision_id") or "").strip() != revision_id:
        raise RuntimeError("connector review receipt revision_id mismatch")
    if str(receipt.get("transport") or "").strip() != "google-drive-connector":
        raise RuntimeError("connector review receipt transport mismatch")
    cloud = receipt.get("cloud_publication") or {}
    if not isinstance(cloud, dict):
        raise RuntimeError("cloud_publication must be an object")
    for key in ("drive_revision_folder_id", "drive_file_id", "cloud_zip_sha256", "bundle_manifest_sha256"):
        if not str(cloud.get(key) or "").strip():
            raise RuntimeError(f"connector review receipt missing cloud_publication.{key}")
    if str(cloud.get("bundle_manifest_sha256")).lower() != str(receipt.get("bundle_manifest_sha256") or "").lower():
        raise RuntimeError("cloud publication manifest SHA does not match review receipt")
    ...
    try:
        ...
        if verdict == "PASS":
            ...
            accepted_revision_id = ""
        output = {
            "schema_version": "openworker-case0003-connector-review-apply/v1",
            "case_id": "0003",
        ...
        out = workspace / "acceptance" / "openworker-final" / f"connector-review-apply-{revision_id}.json"
...
if __name__ == "__main__":
    try:
        ...
        print(f"CASE0003_CONNECTOR_REVIEW_APPLY_FAIL {exc}", file=sys.stderr)
        raise
