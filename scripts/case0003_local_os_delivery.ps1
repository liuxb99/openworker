...
$receipt=[ordered]@{schema='openworker/case0003-os-delivery-submit/v1';case_id='0003';machine=$Machine;workspace_root=$WorkspaceRoot;os_job_id=$OSJobId;approval_total=[int]$approval.total;approval_count=[int]$approval.approved_count;transport='openworker-local-jobs+go-tool-localexec';github_business_transport=$false;submitted_at=[DateTimeOffset]::UtcNow.ToString('o');node=$node;agents=$agents;durable_ack=$ack}
