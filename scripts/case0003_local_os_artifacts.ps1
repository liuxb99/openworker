...
# Require the final local physical gates before registry ingest.
$blenderManifest=Join-Path $WorkspaceRoot 'blender\blender-scene-evidence.json';$sceneXManifest=Join-Path $WorkspaceRoot 'scenex\scenex-workspace.json'
if(-not(Test-Path -LiteralPath $blenderManifest -PathType Leaf)){throw 'Blender REAL evidence gate is not complete'}
if(-not(Test-Path -LiteralPath $sceneXManifest -PathType Leaf)){throw 'SceneX REAL evidence gate is not complete'}
$bev=Get-Content -LiteralPath $blenderManifest -Raw|ConvertFrom-Json;if($bev.schema_version -ne 'blender-scene-evidence/v1'){throw 'Blender evidence schema mismatch'}
$sx=Get-Content -LiteralPath $sceneXManifest -Raw|ConvertFrom-Json;if($sx.schema_version -ne 'scenex-workspace-browse/v1' -or -not $sx.ok -or [int]$sx.active_chunks -le 0 -or [int]$sx.terrain_geometry_count -le 0){throw 'SceneX REAL gate rejected'}
...
$manifest=[ordered]@{schema_version='artifact-ingest/v1';project_id=$OSProjectId;job_id=$OSJobId;artifacts=$items}
