...
$finalizer=Join-Path $OpenWorkerRoot 'scripts\case0003_finalize_reviewed_delivery.py'
if(-not(Test-Path -LiteralPath $finalizer -PathType Leaf)){throw "reviewed delivery finalizer missing: $finalizer"}
...
if([string]::IsNullOrWhiteSpace($RevisionId)){
  ...
  if([string]$review.schema_version -ne 'openworker-case0003-connector-review-apply/v2'){throw 'connector review apply schema mismatch'}
...
$cmd="powershell -NoProfile -ExecutionPolicy Bypass -Command `"Set-Location -LiteralPath '$escapedRoot'; python scripts/case0003_finalize_reviewed_delivery.py --workspace '$escapedWorkspace' --revision-id '$escapedRevision'; if(`$LASTEXITCODE -ne 0){exit `$LASTEXITCODE}`""
