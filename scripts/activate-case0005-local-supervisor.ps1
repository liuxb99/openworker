...
function Resolve-Python([string]$Explicit){
    $candidates=@();if(-not[string]::IsNullOrWhiteSpace($Explicit)){$candidates+=$Explicit};$candidates+=@('C:\Python314\python.exe','C:\Python313\python.exe','C:\Python312\python.exe','C:\Python311\python.exe','C:\Python310\python.exe')
    ...
    $cmd=Get-Command python.exe -ErrorAction SilentlyContinue;if($null-ne $cmd){return $cmd.Source};throw 'Python executable not found'
}

$OpenWorkerRoot=Resolve-RepoRoot -Explicit $OpenWorkerRoot -Name 'OpenWorker' -Markers @('case-specs\0005.json','coworker\case0005_verified_local_controller.py') -Candidates @('C:\github-runners\openworker\_work\openworker\openworker','D:\AI\openworker','D:\AIWork\openworker','D:\PyWork\openworker')
$GoToolRoot=Resolve-RepoRoot -Explicit $GoToolRoot -Name 'go-tool-runtime' -Markers @('go.mod','scripts\windows\install-and-verify-true-local-supervisor.ps1','scripts\windows\verify-gtr-true-local-supervisor.ps1') -Candidates @('C:\github-runners\go-tool-runtime\_work\go-tool-runtime\go-tool-runtime','D:\AI\go-tool-runtime','D:\AIWork\go-tool-runtime','D:\PyWork\go-tool-runtime')
$EngineeringOSRoot=Resolve-RepoRoot -Explicit $EngineeringOSRoot -Name 'AI-Engineering-OS' -Markers @('go.mod','AI-Engineering-OS.bat','bootstrap-ai-engineering-os.bat') -Candidates @('C:\github-runners\AI-Engineering-OS\_work\AI-Engineering-OS\AI-Engineering-OS','D:\AI\AI-Engineering-OS','D:\AIWork\AI-Engineering-OS','D:\PyWork\AI-Engineering-OS')
$PythonExe=Resolve-Python $PythonExe

# Git is code synchronization only; no workflow dispatch is used.
if(-not $SkipCodeSync){
    foreach($repo in @($GoToolRoot,$OpenWorkerRoot,$EngineeringOSRoot)){
        if(Test-Path -LiteralPath (Join-Path $repo '.git') -PathType Container){
            Push-Location $repo
            try{$dirty=(& git status --porcelain);if($LASTEXITCODE-ne 0){throw "git status failed: $repo"};if(-not[string]::IsNullOrWhiteSpace(($dirty-join "`n"))){throw "refuse code sync on dirty checkout: $repo"};& git pull --ff-only origin main;if($LASTEXITCODE-ne 0){throw "git pull --ff-only failed: $repo"}}finally{Pop-Location}
        }
    }
}

# Persist machine-local tool identity before starting the executor. The setter is
# merge-safe and preserves unrelated Terrain/SceneX/Drive roots.
$setRoots=Join-Path $OpenWorkerRoot 'scripts\openworker_set_machine_roots.ps1'
& powershell.exe -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $setRoots -OpenWorkerRoot $OpenWorkerRoot -GoToolRoot $GoToolRoot -EngineeringOSRoot $EngineeringOSRoot
if($LASTEXITCODE-ne 0){throw "machine-root authority update failed: $LASTEXITCODE"}

# Always rebuild/reinstall current binaries and rerun REAL verification.
...
$specPath=Join-Path $OpenWorkerRoot 'case-specs\0005.json';$manifestPath=Join-Path $OpenWorkerRoot 'case-worklists\0005.json';$controllerModule='coworker.case0005_verified_local_controller'
$bootstrap=[ordered]@{case_id='0005';machine=$expectedHost;workspace_root=$Workspace;openworker_root=$OpenWorkerRoot;controller_module=$controllerModule;manifest_path=$manifestPath;spec_path=$specPath;python_exe=$PythonExe;env=[ordered]@{GTR_WORK_QUEUE_URL='http://127.0.0.1:8848';GTR_LOCAL_WORKERS='4';OPENWORKER_ROOT=$OpenWorkerRoot;GO_TOOL_ROOT=$GoToolRoot;ENGINEERING_OS_ROOT=$EngineeringOSRoot;GTR_LOCAL_EXEC_EXE=(Join-Path $installRoot 'gtr-local-exec.exe')}}
...
if([string]$bootstrapAck.status-ne 'completed'){throw "Case 0005 local bootstrap failed: $($bootstrapAck|ConvertTo-Json -Depth 8 -Compress)"};if([bool]$bootstrapAck.github_action_used_for_business_execution){throw 'Case bootstrap unexpectedly reports GitHub business execution'}
