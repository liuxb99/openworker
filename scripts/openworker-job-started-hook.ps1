...
if([string]::IsNullOrWhiteSpace($secret)){
  ...
  Write-HookReceipt -Receipt ([ordered]@{schema='openworker.hook-receipt.v1';mode='passthrough';machine=[Environment]::MachineName;accepted=$true;exit_code=0;started_at=$started.ToString('o');completed_at=[DateTimeOffset]::UtcNow.ToString('o')})
  exit 0
}

Write-Host '[OpenWorker Hook] OPENWORKER_CONTROL detected'
try{$envelope=$secret|ConvertFrom-Json -ErrorAction Stop}catch{
  Write-HookReceipt -Receipt ([ordered]@{schema='openworker.hook-receipt.v1';mode='control';machine=[Environment]::MachineName;accepted=$false;exit_code=80;error_class='invalid_json';error=$_.Exception.Message;started_at=$started.ToString('o');completed_at=[DateTimeOffset]::UtcNow.ToString('o')})
...
$fail={param([int]$Code,[string]$Class,[string]$Message)
  Write-HookReceipt -Receipt ([ordered]@{schema='openworker.hook-receipt.v1';mode='control';request_id=$requestId;command=[string]$envelope.command;machine=[Environment]::MachineName;accepted=$false;exit_code=$Code;error_class=$Class;error=$Message;started_at=$started.ToString('o');completed_at=[DateTimeOffset]::UtcNow.ToString('o')})
...
if([string]::IsNullOrWhiteSpace($root)){
  foreach($candidate in @((Join-Path $env:ProgramData 'OpenWorker\repo'),'C:\github-runners\openworker\_work\openworker\openworker','D:\AI\openworker','D:\AIWork\openworker','D:\PyWork\openworker')){
...
if([string]::IsNullOrWhiteSpace($root)){&$fail 87 'openworker_root_missing' 'OpenWorker repo root not found'}
...
$temp=Join-Path $env:TEMP ("openworker-control-$requestId.json")
try{
