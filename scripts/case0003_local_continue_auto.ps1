...
foreach($required in @($controller,$preflight,$imageryQuarantine,$terrainQuarantine,$consumerQuarantine,$renderQuarantine,$imageryRecorder,$terrainRecorder)){if(-not(Test-Path -LiteralPath $required -PathType Leaf)){throw "Case 0003 required entrypoint missing: $required"}}
...
if(Test-Path -LiteralPath $continueReceipt -PathType Leaf){$state=Get-Content -LiteralPath $continueReceipt -Raw|ConvertFrom-Json;if([string]$state.schema -eq 'openworker/case0003-local-continue/v10'){Push-Location $OpenWorkerRoot;try{if([bool]$state.gates_after_submission.streetview -and [bool]$state.gates_after_submission.orthophoto){python scripts/case0003_record_imagery_acceptance.py --workspace $WorkspaceRoot;if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}};if([bool]$state.gates_after_submission.terrain){python scripts/case0003_record_terrain_acceptance.py --workspace $WorkspaceRoot --catalog $CatalogPath;if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}};if((Test-Path -LiteralPath $renderRecorder -PathType Leaf)-and[bool]$state.gates_after_submission.blender -and[bool]$state.gates_after_submission.scenex){python scripts/case0003_record_render_acceptance.py --workspace $WorkspaceRoot;if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}}}finally{Pop-Location}}}
